...
contract STD_E_x6 is AccessControl, StdReferenceBase {
    ...
    function getReferenceData(uint256 symbolBaseQuote) public view returns (ReferenceData memory) {
        (uint256 baseRate, uint256 baseLastUpdate) = _getRefData((symbolBaseQuote & 65280) >> 8);
        (uint256 quoteRate, uint256 quoteLastUpdate) = _getRefData((symbolBaseQuote & 255));
        return ReferenceData({rate: (baseRate * 1e18) / quoteRate, lastUpdatedBase: baseLastUpdate, lastUpdatedQuote: quoteLastUpdate});
    }

    function _getRefData(uint256 symbol) internal view returns (uint256 rate, uint256 lastUpdate) {
        // > 9999 is USD
        if (symbol > 9999) {
    ...
    function getReferenceData(string memory base, string memory quote) public view override returns (ReferenceData memory x) {
        require(false, "do nothing");
        require(keccak256(abi.encode(base)) != keccak256(abi.encode(quote)), "???");
        require(x.rate != 0 && block.timestamp > 0, "???");
    }
